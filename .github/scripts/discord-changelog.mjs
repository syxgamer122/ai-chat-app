...
if (nhanh !== nhanhChinh) {
  console.log(
    `Push vào nhánh "${nhanh}", không phải nhánh chính "${nhanhChinh}" — bỏ qua.`,
  );
  process.exit(0);
}
...
const nhanNhom = (msg) =>
  NHOM.find(([re]) => re.test(msg))?.[1] ?? "📦 Thay đổi khác";
...
for (const c of commits.slice(0, GIOI_HAN)) {
  ...
  theoNhom
    .get(nhan)
    .push(`• [\`${c.id.slice(0, 7)}\`](${c.url}) ${gonGang(c.message)}`);
}

const phan = [...theoNhom].map(
  ([nhan, dong]) => `**${nhan}**\n${dong.join("\n")}`,
);
...
const embed = {
  ...
  footer: {
    text: `${nguoiDay} đẩy code • bấm tiêu đề để xem toàn bộ thay đổi`,
  },
};
