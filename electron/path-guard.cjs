...
function resolveWithin(root, relPath) {
  if (typeof root !== 'string' || root.length === 0) {
    throw new Error('Workspace root chưa được chọn.');
  }
  if (typeof relPath !== 'string') {
    throw new Error('Đường dẫn phải là string.');
  }
  if (relPath.length > 1024) {
    throw new Error('Đường dẫn quá dài (>1024 ký tự).');
  }
  ...
  if (path.isAbsolute(clean) || /^[a-zA-Z]:/.test(clean) || clean.startsWith('\\\\') || clean.startsWith('//')) {
    throw new Error(`Đường dẫn tuyệt đối bị cấm trong workspace: ${clean.slice(0, 80)}`);
  }
  if (clean.includes('\0')) {
    throw new Error('Đường dẫn chứa ký tự NUL.');
  }
  const rootAbs = path.resolve(root);
  const target = path.resolve(rootAbs, clean);
  if (!isWithinRoot(rootAbs, target)) {
    throw new Error(`Đường dẫn thoát khỏi workspace: ${clean.slice(0, 80)}`);
  }
  return target;
}
