 ...
 *
 * Lý do (residual B1(a) từ `CRITIQUE_RECONCILIATION.md` mục A1): kế thừa PATH của
...
/**
 * PATH "an toàn" thay cho PATH kế thừa: thư mục hệ thống + `node_modules/.bin`
 * của workspace (cần cho npx/npm chạy tool cục bộ — ĐÂY là lý do duy nhất ta
 * vẫn phải dựng PATH thay vì bỏ hẳn: npm script tự cần PATH để tìm `node`).
 */
function buildSafePath(workspaceRoot) {
  const dirs = [];
  if (workspaceRoot) {
    dirs.push(path.join(workspaceRoot, 'node_modules', '.bin'));
  }
...
function getSafeEnv(workspaceRoot) {
  ...
  // Windows: các lệnh npm/yarn là script .cmd — Node cần ComSpec để chạy shim,
  // nên PATH phải có PATHEXT để spawn tìm thấy (không dùng shell).
