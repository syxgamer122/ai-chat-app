...
const { BgJobStore, jobsDir, MAX_RUNNING } = require('./bg-jobs.cjs');
let z;
try {
  ...
  z = {
    ...
    object: (schema) => ({
      parse: (val) => {
        if (!val || typeof val !== 'object') throw new Error('Expected object payload');
        return val;
      },
      refine: (fn, err) => ({
        parse: (val) => {
          if (!val || typeof val !== 'object') throw new Error('Expected object payload');
          if (fn && !fn(val)) throw new Error(typeof err === 'string' ? err : err?.message || 'Validation failed');
          return val;
        },
      }),
    }),
...
const { compileShellCommand, getSafeEnv, killProcessTree, resolveBinaryAbsolute } = require('./shell-policy.cjs');
const { consumeApprovalToken } = require('./approval-binding.cjs');
...
const LIMITS = Object.freeze({
  ...
  /* Smart truncation: giữ output trong ngân sách token của LLM.
...
const FsWritePayload = z.object({
  ...
  expectedBaseHash: z.string().max(128).optional(),
  hasDiff: z.boolean().optional(),
}).refine(
  (data) => !data.hasDiff || (typeof data.expectedBaseHash === 'string' && data.expectedBaseHash.length > 0),
  {
    message: 'expectedBaseHash là bắt buộc đối với mọi mutation đĩa khi có diff (TOCTOU protection).',
    path: ['expectedBaseHash'],
  }
);
...
const BgRunPayload = z.object({
  command: z.string().min(1).max(500),
  timeout_secs: z.number().int().min(1).max(3600).optional(),
});
const BgStatusPayload = z.object({ id: z.string().min(1).max(80).optional() });
const BgStopPayload = z.object({ id: z.string().min(1).max(80) });
/** Trần ký tự output trả cho model trong bg_status (log đầy đủ vẫn ở file). */
const BG_OUTPUT_TAIL_CHARS = 2000;
...
async function fsWrite(payload) {
  ...
  if (payload.hasDiff && (!payload.expectedBaseHash || typeof payload.expectedBaseHash !== 'string')) {
    throw new Error('expectedBaseHash là bắt buộc đối với mọi mutation đĩa khi có diff (TOCTOU protection).');
  }

  // 2.1 Content-addressed TOCTOU check
  if (typeof payload.expectedBaseHash === 'string' && payload.expectedBaseHash.length > 0) {
    try {
      const existing = await fsp.readFile(file);
      const currentHash = crypto.createHash('sha256').update(existing).digest('hex');
      if (currentHash !== payload.expectedBaseHash) {
        throw new Error(
          `TOCTOU Conflict: File "${payload.relPath}" đã bị thay đổi trên đĩa kể từ khi diff được duyệt (expected: ${payload.expectedBaseHash.slice(0, 8)}, current: ${currentHash.slice(0, 8)}). Ghi bị hủy để bảo vệ dữ liệu.`
        );
      }
    } catch (err) {
      if (err.code !== 'ENOENT') throw err;
      throw new Error(`TOCTOU Conflict: File "${payload.relPath}" không tồn tại trên đĩa như mong đợi.`);
    }
  }

  // 2.1 Atomic write: ghi ra temp file cùng thư mục rồi rename
  const tmpId = crypto.randomBytes(4).toString('hex');
  const tmpFile = path.join(dir, `.vyen_tmp_${Date.now()}_${tmpId}.tmp`);
  try {
    await fsp.writeFile(tmpFile, payload.content, 'utf8');
...
const runningChildrenByChatId = new Map();

/* Lệnh nền (bg_run): handle child sống trong map này để bg_stop diệt được;
   sau restart map rỗng — bg_stop diệt theo pid từ meta trên đĩa. */
let bgStore = null;
const bgChildren = new Map();
...
function killByChatId(chatId) {
  let count = 0;
  if (!chatId || typeof chatId !== 'string') return { killed: 0 };
  const set = runningChildrenByChatId.get(chatId);
  if (set) {
    for (const child of set) {
      killTree(child);
      count++;
    }
    runningChildrenByChatId.delete(chatId);
  }
  return { killed: count };
}
...
function killAllRunning() {
  ...
  runningChildrenByChatId.clear();
}

/* ------------------------------------------------------------------ */
/* Smart output truncation (P2)                                */
...
function shellRun(payload) {
  // C-03: Enforce approval token verification at privileged bridge before executing commands
  if (!payload.approvalToken || typeof payload.approvalToken !== 'string') {
    const err = new Error('approval_required: Lệnh shell đòi hỏi approval token hợp lệ từ người dùng.');
  ...
  const expectedBinding = {
    ...
    ...(payload.workspaceFingerprint ? { workspaceFingerprint: payload.workspaceFingerprint } : {}),
  };

  const verification = consumeApprovalToken(payload.approvalToken, expectedBinding);
  if (!verification.ok) {
    if (verification.reason === 'approval_replayed') {
      const err = new Error('approval_replayed: Token đã được sử dụng trước đó (chống replay).');
      err.code = 'approval_replayed';
      throw err;
    }
    const err = new Error(`approval_required: Token phê duyệt không hợp lệ (${verification.reason}).`);
    err.code = 'approval_required';
    throw err;
  }
  ...
  // 1.1 Shell Policy: Compile argv and validate allowlist (rejects shell operators & metacharacters)
  let compiled;
  try {
    compiled = compileShellCommand(payload.command);
  } catch (err) {
    return Promise.resolve({
      code: 1,
      signal: null,
      stdout: '',
      stderr: `[SHELL POLICY VIOLATION] ${err.message}`,
      durationMs: 0,
      timedOut: false,
      truncated: false,
    });
  ...
  // Direct Node execution bypass on Windows: Chạy trực tiếp file .js qua node.exe
  // Bỏ qua hoàn toàn wrapper .cmd và cmd.exe để triệt tiêu Second-Order Parameter Expansion
  let directNodeCli = null;
  if (isWin && (binName === 'npm' || binName === 'npx' || binName === 'corepack')) {
    const nodeDir = path.dirname(process.execPath);
    const cliCandidate = binName === 'npm'
      ? path.join(nodeDir, 'node_modules', 'npm', 'bin', 'npm-cli.js')
      : binName === 'npx'
      ? path.join(nodeDir, 'node_modules', 'npm', 'bin', 'npx-cli.js')
      : path.join(nodeDir, 'node_modules', 'corepack', 'dist', 'corepack.js');
    if (fs.existsSync(cliCandidate)) {
      directNodeCli = { bin: process.execPath, args: [cliCandidate, ...compiled.args] };
    }
  }

  if (isWin && !directNodeCli && (binName === 'npm' || binName === 'pnpm' || binName === 'npx' || binName === 'yarn')) {
    binName = `${binName}.cmd`;
  }

  /*
   * S3 (residual B1(a)): resolve binary về ĐƯỜNG DẪN TUYỆT ĐỐI trong thư mục hệ
   * thống, không để `spawn` tra PATH kế thừa của user. Không tìm thấy ⇒ từ chối
   * (không fallback, không im lặng chạy binary lạ).
   */
  let bin = directNodeCli ? directNodeCli.bin : resolveBinaryAbsolute(binName);
  if (!bin && isWin) {
    if (binName === 'echo' || binName === 'dir') {
      bin = path.join(process.env.SystemRoot || 'C:\\Windows', 'System32', 'cmd.exe');
      compiled.args = ['/d', '/c', binName, ...compiled.args];
    } else {
      const nodeDir = path.dirname(process.execPath);
  ...
  if (!bin) {
    return Promise.resolve({
      ...
      signal: null,
      stdout: '',
      stderr: `[SHELL POLICY] Không tìm thấy binary "${binName}" trong thư mục hệ thống tin cậy.`,
      durationMs: 0,
      timedOut: false,
      truncated: false,
    });
  ...
  let spawnBin = directNodeCli ? directNodeCli.bin : bin;
  let spawnArgs = directNodeCli ? directNodeCli.args : compiled.args;
  if (isWin && !directNodeCli && bin && (bin.toLowerCase().endsWith('.cmd') || bin.toLowerCase().endsWith('.bat'))) {
    spawnBin = path.join(process.env.SystemRoot || 'C:\\Windows', 'System32', 'cmd.exe');
    spawnArgs = ['/d', '/c', bin, ...compiled.args];
  }

  // PATH dựng lại: thư mục hệ thống tin cậy (C-01: không còn chứa node_modules/.bin).
  const safeEnv = getSafeEnv(root);
  ...
  return new Promise((resolve) => {
    ...
    if (payload.chatId) {
      let set = runningChildrenByChatId.get(payload.chatId);
      if (!set) {
        set = new Set();
        runningChildrenByChatId.set(payload.chatId, set);
      }
      set.add(child);
    }
    auditLog(`shell: ${payload.command.slice(0, 200)} (bin=${bin}, cwd=${path.relative(root, cwd) || '.'}, timeout=${timeoutMs}ms)`);
    ...
    child.on('error', (err) => {
      ...
      if (payload.chatId) {
        const set = runningChildrenByChatId.get(payload.chatId);
        if (set) {
          set.delete(child);
          if (set.size === 0) runningChildrenByChatId.delete(payload.chatId);
        }
      }
 ...
 * Origin nên gateway allowlist-origin ("Origin not allowed") không chặn
...
function register(ipcMain, opts) {
  ...
  bgStore = new BgJobStore(jobsDir(opts.userDataDir));
  ...
  on('vyen:shell-kill-by-chat', (p) => {
    const schema = z.object({ chatId: z.string() });
    const { chatId } = schema.parse(p ?? {});
    return killByChatId(chatId);
  });

  /* Skills toàn cục ~/.vyen/skills (P0-3): chỉ đọc đúng thư mục
     này, KHÔNG mở quyền fs tùy ý — name bị chặn regex trước khi join path. */
  on('vyen:home-skills-list', async () => {
    const root = path.join(os.homedir(), '.vyen', 'skills');
    const out = [];
    let entries = [];
    try {
    ...
    for (const e of entries) {
      ...
      try {
        const st = await fsp.stat(path.join(root, e.name, 'SKILL.md'));
        if (st.isFile()) out.push({ name: e.name, path: `${e.name}/SKILL.md` });
      } catch { /* thư mục thiếu SKILL.md — bỏ qua */ }
    }
    return { skills: out };
  });
  on('vyen:home-skills-read', async (payload) => {
    const schema = z.object({ name: z.string().min(1).max(60) });
    const p = schema.parse(payload);
    if (!/^[a-zA-Z0-9][\w.-]*$/.test(p.name)) throw new Error('Tên skill không hợp lệ.');
    const dir = path.join(os.homedir(), '.vyen', 'skills', p.name);
    const skillPath = path.join(dir, 'SKILL.md');
    const st = await fsp.stat(skillPath);
    if (!st.isFile()) throw new Error('SKILL.md không phải file.');
    if (st.size > LIMITS.FS_READ_MAX_BYTES) {
      throw new Error(`SKILL.md quá lớn (${st.size} bytes).`);
    }
  ...
  /* Mirror bộ nhớ toàn cục ~/.vyen/memory/<category>.md (P1-4) — cùng mẫu
     khoá an toàn như home-skills: category chặn regex, chỉ trong thư mục này. */
  on('vyen:home-memory-write', async (payload) => {
    const schema = z.object({
      category: z.string().min(1).max(60),
      content: z.string().max(200_000),
    });
    const p = schema.parse(payload);
    if (!/^[a-zA-Z0-9][\w.-]*$/.test(p.category)) throw new Error('Tên category không hợp lệ.');
    const dir = path.join(os.homedir(), '.vyen', 'memory');
    await fsp.mkdir(dir, { recursive: true });
    await fsp.writeFile(path.join(dir, `${p.category}.md`), p.content, 'utf8');
    return { ok: true };
  });
  on('vyen:home-memory-read', async (payload) => {
    const schema = z.object({ category: z.string().min(1).max(60) });
    const p = schema.parse(payload);
    if (!/^[a-zA-Z0-9][\w.-]*$/.test(p.category)) throw new Error('Tên category không hợp lệ.');
    const file = path.join(os.homedir(), '.vyen', 'memory', `${p.category}.md`);
    const content = await fsp.readFile(file, 'utf8');
    return { content };
  });

  on('vyen:bg-run', (p) => {
    const payload = BgRunPayload.parse(p);
    const root = requireRoot();
    if (bgStore.runningCount() >= MAX_RUNNING) {
      throw new Error(`Đã có ${MAX_RUNNING} lệnh nền đang chạy — hỏi bg_status, bg_stop bớt rồi mới chạy thêm.`);
    }
    const cwd = resolveWithin(root, '');
    const id = bgStore.newId();
    bgStore.ensureDir();
    const fd = fs.openSync(bgStore.logPath(id), 'a');
    const isWin = process.platform === 'win32';
    ...
    const child = spawn(file, args, {
      ...
      // detached + unref: process con SỐNG SÓT qua restart server; output chảy
      // thẳng vào file log nên không cần giữ pipe.
      detached: true,
      stdio: ['ignore', fd, fd],
      env: { ...process.env },
    });
    ...
    const timeoutSecs = payload.timeout_secs ?? 3600;
    bgStore.create({ id, command: payload.command, pid: child.pid, timeoutSecs });
    auditLog(`bg-run: ${payload.command.slice(0, 200)} (id=${id}, timeout=${timeoutSecs}s)`);
    const timer = setTimeout(() => {
      if (child.exitCode !== null) return;
      killTree(child);
      bgStore.update(id, { status: 'stopped', note: 'Hết timeout bg_run — đã dừng.' });
    }, timeoutSecs * 1000);
    timer.unref?.();
    let fdClosed = false;
    const closeFdOnce = () => {
    ...
    child.on('exit', (code, signal) => {
      closeFdOnce();
      bgStore.update(id, {
        status: signal ? 'stopped' : code === 0 ? 'done' : 'failed',
        exitCode: code,
        finishedAt: Date.now(),
      });
    });
    child.on('error', () => {
      closeFdOnce();
      bgStore.update(id, { status: 'failed', exitCode: null, note: 'Không spawn được process.' });
    });
    return {
      jobId: id,
      pid: child.pid,
      note: 'Đang chạy nền. Hỏi bg_status với job_id này để xem tiến độ; bg_stop để dừng.',
    };
  });

  on('vyen:bg-status', (p) => {
    const payload = BgStatusPayload.parse(p ?? {});
    // Recovery sau restart: job còn 'running' mà pid đã chết → chốt 'done'.
    bgStore.reconcile((pid) => {
      try {
        process.kill(pid, 0);
        return true;
      } catch {
    ...
    if (payload.id) {
      const job = bgStore.get(payload.id);
      if (!job) throw new Error(`Job "${payload.id}" không tồn tại (có thể đã bị prune).`);
      return { jobs: [{ ...job, outputTail: bgStore.readTail(payload.id, BG_OUTPUT_TAIL_CHARS) }] };
    }
    bgStore.prune();
    const jobs = bgStore
      .list()
      .slice(0, 20)
      .map((j) => ({ ...j, outputTail: bgStore.readTail(j.id, BG_OUTPUT_TAIL_CHARS) }));
    return { jobs };
  });

  on('vyen:bg-stop', (p) => {
    const { id } = BgStopPayload.parse(p);
    const job = bgStore.get(id);
    if (!job) throw new Error(`Job "${id}" không tồn tại.`);
    if (job.status !== 'running') return { ok: true, note: `Job đã ${job.status} trước đó.` };
    const child = bgChildren.get(id);
    if (child) {
      killTree(child);
    } else if (typeof job.pid === 'number') {
      // Sau restart không còn handle — diệt theo pid từ meta.
      if (process.platform === 'win32') {
        spawnSync('taskkill', ['/pid', String(job.pid), '/T', '/F'], { windowsHide: true });
      } else {
        try { process.kill(job.pid, 'SIGTERM'); } catch { /* đã chết */ }
      }
    }
    bgStore.update(id, { status: 'stopped', finishedAt: Date.now() });
    auditLog(`bg-stop: ${id}`);
    return { ok: true };
  });
