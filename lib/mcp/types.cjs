 ...
 *
 * Shared giữa lib/mcp/ modules. Zod v3 compatible.
 *
...
try {
  ...
  const createStub = () => {
    ...
    target.parse = (val) => val;
    target.safeParse = (val) => ({ success: true, data: val });
...
const commonFields = {
  ...
  /** Whitelist tool theo từng server (available_tools). */
  availableTools: z.array(z.string().max(200)).optional(),
  available_tools: z.array(z.string().max(200)).optional(),
  /** Timeout mỗi lần gọi tool, giây. Mặc định 60 (khớp DEFAULT_REQUEST_TIMEOUT_MSEC của SDK). */
  timeoutSecs: TimeoutSecsSchema,
  /**
   * Cách đưa tool vào ngữ cảnh model. 'full' (mặc định, khi thiếu field):
   * schema đầy đủ của mọi tool nằm trong mỗi request. 'proxy': tool KHÔNG
   * được khai báo trực tiếp — chỉ có một tool mcp__search để search/describe/
   * call, đổi bớt token ngữ cảnh lấy thêm một lượt gọi trung gian.
   */
  exposeMode: z.enum(['full', 'proxy']).optional(),
};
...
const McpSetExposeModePayload = z.object({
  id: ServerIdSchema,
  exposeMode: z.enum(['full', 'proxy']),
});
...
/**
 * File policy lưu dưới dạng {"<serverId>:<toolName>:<schemaHash>": "<decision>"}.
 ...
 * Độ dài key tối đa: serverId (64) + toolName (200) + schemaHash (64) + delimiters (2) <= 330 -> max 400.
 */
