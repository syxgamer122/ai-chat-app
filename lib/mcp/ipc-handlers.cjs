...
function computeSchemaHash(tool) {
  if (!tool) {
...
const approvalPolicies = new Map(); // key: "serverId:toolName:schemaHash"
 ...
 */
function createApproval(serverId, toolName, args, schemaHash) {
  const id = randomUUID();
  ...
  return new Promise((resolve) => {
    ...
    pendingApprovals.set(id, { id, serverId, toolName, args, schemaHash: schemaHash || '', createdAt, settle });
...
function resolveApproval(approvalId, decision) {
  ...
  if (decision === 'always_allow' || decision === 'always_deny') {
    const grantKey = `${pending.serverId}:${pending.toolName}:${pending.schemaHash || ''}`;
    approvalPolicies.set(grantKey, decision);
    persistPolicies();
  }
...
function checkPolicy(serverId, toolName, schemaHash) {
  if (schemaHash === undefined && manager) {
    const serverEntry = manager.servers.get(serverId);
    const tool = serverEntry?.tools?.find((t) => t.name === toolName);
    schemaHash = computeSchemaHash(tool);
  }
  const grantKey = `${serverId}:${toolName}:${schemaHash || ''}`;
  return approvalPolicies.get(grantKey) ?? null;
}
...
async function register(ipcMain, opts) {
  ...
  on('mcp:set-expose-mode', async (payload) => {
    const { id, exposeMode } = McpSetExposeModePayload.parse(payload);
    const cfg = manager.getConfigs().find((c) => c.id === id);
    if (!cfg) throw new Error(`Server "${id}" không tồn tại.`);
    // Mutate trực tiếp entry.config (cùng tham chiếu manager đang giữ) —
    // KHÔNG reconnect: exposeMode chỉ đổi cách đẩy tool vào ngữ cảnh model,
    // transport/tools không đổi.
    cfg.exposeMode = exposeMode;
    persistConfigs();
    auditLog(`MCP expose mode: ${id} → ${exposeMode}`);
    return { ok: true };
  });
  ...
  on('mcp:call-tool', async (payload) => {
    ...
    const policy = checkPolicy(serverId, toolName, schemaHash);
    ...
    if (policy !== 'always_allow' && !manager.isAutoApproved(serverId, toolName)) {
      const decision = await createApproval(serverId, toolName, args, schemaHash);
...
module.exports = { register, shutdown, APPROVAL_TIMEOUT_MS, computeSchemaHash, canonicalJson };
