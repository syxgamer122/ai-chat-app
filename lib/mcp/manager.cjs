...
class McpManager extends EventEmitter {
  ...
  listTools() {
    ...
    for (const [id, entry] of this.servers) {
      ...
      const whitelist = entry.config.availableTools || entry.config.available_tools;
      for (const tool of entry.tools) {
        if (Array.isArray(whitelist) && whitelist.length > 0 && !whitelist.includes(tool.name)) {
          continue;
        }
        tools.push({
          name: tool.name,
  ...
  async callTool(serverId, toolName, args) {
    ...
    const whitelist = entry.config.availableTools || entry.config.available_tools;
    if (Array.isArray(whitelist) && whitelist.length > 0 && !whitelist.includes(toolName)) {
