...
class McpManager extends EventEmitter {
  ...
  /**
   * Trích xuất và chuẩn hóa whitelist available_tools (Port Goose).
   * @param {import('./types.cjs').McpServerConfig} config
   * @returns {string[] | null}
   */
  _getEffectiveWhitelist(config) {
    if (!config) return null;
    const raw = config.availableTools || config.available_tools;
    if (!Array.isArray(raw)) return null;
    const cleaned = raw.map((s) => (typeof s === 'string' ? s.trim() : String(s))).filter(Boolean);
  ...
  listTools() {
    ...
    for (const [id, entry] of this.servers) {
      ...
      const whitelist = this._getEffectiveWhitelist(entry.config);
  ...
  async callTool(serverId, toolName, args) {
    ...
    if (whitelist && !whitelist.includes(toolName)) {
      throw new Error(`Tool "${toolName}" không nằm trong danh sách available_tools của server "${serverId}".`);
    }
  ...
  getStatuses() {
    ...
    for (const [id, entry] of this.servers) {
      const whitelist = this._getEffectiveWhitelist(entry.config);
      const activeToolCount = whitelist
        ? entry.tools.filter((t) => whitelist.includes(t.name)).length
        : entry.tools.length;
      statuses.push({
        id,
        ...
        exposeMode: entry.config.exposeMode === 'proxy' ? 'proxy' : 'full',
      });
    }
    return statuses;
  }
