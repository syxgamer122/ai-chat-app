...
## Architecture
Vyen is a high-assurance multi-agent coding harness running dual-mode (headless CLI, desktop WebView `--app`, và Universal Web bridge). Dự án tổng hợp các mẫu kiến trúc tiên tiến vào `lib/teamwork/`:
...
|---|------|-------|-------------|--------|
| M1 | Durable DAG Workflow Engine & Checkpointing | `lib/teamwork/dag/`, `lib/teamwork/checkpoint/` | none | IMPLEMENTED |
| M2 | Human-in-the-Loop Approval & Visual Inspection | `lib/teamwork/hitl/`, `lib/teamwork/visual/` | none | IMPLEMENTED |
| M3 | Strict Tool Contracts, Provenance & Process Sandbox | `lib/teamwork/contracts/`, `lib/teamwork/sandbox/` | none | IMPLEMENTED |
| M4 | Temporal Context Memory & Bitemporal Codebase Ledger | `lib/teamwork/ledger/`, `lib/teamwork/context/` | none | IMPLEMENTED |
| M5 | Engine Integration, CLI Runner & Full Regression Pass | `lib/teamwork/engine.ts`, `lib/teamwork/tools.ts`, `lib/teamwork/cli.ts`, `bin/teamwork.ts` | M1, M2, M3, M4 | IMPLEMENTED |
| E2E | E2E Testing Suite (Tiers 1-4) | `tests/teamwork-e2e.test.ts`, `tests/teamwork-e2e-architectures.test.ts`, `tests/teamwork-e2e-scenarios.test.ts`, `tests/teamwork-engine-integrated.test.ts` | parallel with M1-M5 | IMPLEMENTED |

> **Đồng bộ 2026-09-24**: mọi thư mục trong `lib/teamwork/` và 24 suite `tests/teamwork-*.test.ts` đã
> có trong repo — trạng thái `PLANNED` trước đây là mốc của giai đoạn lập kế hoạch. Các tên file
> `tests/e2e-*.test.ts` trong bản cũ chưa từng được tạo; tên đúng nằm ở hàng E2E phía trên.
> Lưu ý: workspace này chưa chạy lại toàn bộ suite (không có `node_modules`), nên `IMPLEMENTED`
> nghĩa là **code + test có mặt**, không phải xác nhận PASS.
