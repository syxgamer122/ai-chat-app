...
import type { BranchInfo } from '@/components/chat/message-item';
...
export interface UseChatOrchestrationOptions {
  ...
  agentRuntime?: UseAgentRuntimeReturn;
}
...
export function useChatOrchestration(options?: UseChatOrchestrationOptions) {
  ...
  const applyAllStaged = useCallback(async () => {
    ...
    for (const file of files) {
      try {
        ...
        const currentDiskHash = currentDiskText !== null ? await computeSha256(currentDiskText) : null;
  ...
  const autoApproveShell = useCallback(
    async (s: { command: string; cwd?: string }): Promise<{ approved: boolean; token?: string }> => {
      ...
      if (
        ...
        const autoToken = createApprovalToken({
          kind: 'shell',
          payload: { command: s.command, cwd: s.cwd },
          chatId: chatKey,
          activeLeafId: activeLeafIdRef.current ?? undefined,
  ...
  const autoApproveDiff = useCallback(
    async (s: { path: string; oldText: string; newText: string; toolName?: string; existedBefore?: boolean; expectedBaseHash?: string }): Promise<boolean> => {
  ...
  const workspaceReconnectRequired = Boolean(
    sessionWorkspacePath && (!workspace || workspace.name !== sessionWorkspacePath),
  );
 ...
 */
    ...
    async ({ toolCall }: { toolCall: { toolName: string; args?: unknown } }) => {
      ...
      const runPostEditChecks = async (): Promise<PostEditCheckOutcome[] | null> => {
        ...
        for (const c of commands) {
          let approval: { approved: boolean; token?: string } = { approved: false };
          try {
            approval = await autoApproveShell({ command: c.command, cwd: undefined });
          } catch {
            approval = { approved: false };
          }
          if (!approval.approved) break; // policy từ chối → dừng cả vòng, result giữ nguyên
          try {
            const bridge = (await import('@/lib/desktop-bridge')).vyenDesktop()!;
            const r = await bridge.shell.run({
              command: c.command,
              timeoutMs: POST_EDIT_CHECK_TIMEOUT_MS,
              approvalToken: approval.token, chatId: chatKey,
            });
      ...
      try {
        switch (toolCall.toolName) {
          ...
          case 'fs_edit': {
            ...
            const approved = await autoApproveDiff({ path, oldText: beforeText, newText: current, expectedBaseHash: baseHash ?? undefined });
            if (!approved) {
              return JSON.stringify({
                applied: false,
                approved: false,
            ...
            const res = isDesktop ? await desktopFsWrite(path, current, baseHash ?? undefined, true) : await fsWrite(wsForFs!, path, current, baseHash ?? undefined);
          ...
          case 'fs_write': {
            ...
            const baseHash = fileOnDiskExists ? await computeSha256(oldText) : undefined;
            ...
            const diskCheck = isDesktop ? await desktopFsReadFull(path) : await fsReadFull(wsForFs!, path);
            const currentDiskText = diskCheck.status === 'ok' ? diskCheck.content : null;
            const currentDiskHash = currentDiskText !== null ? await computeSha256(currentDiskText) : undefined;
            if (currentDiskHash !== baseHash) {
              return JSON.stringify({
                written: false,
          ...
          case 'shell_run': {
            ...
            const approval = await autoApproveShell({ command, cwd });
          ...
          case 'git_add': {
            ...
            if (targetsProtectedPath('git_add', { paths })) {
              const approval = await autoApproveShell({
          ...
          case 'code_patch': {
            ...
            const res = isDesktop ? await desktopFsWrite(targetPath, patched.modifiedContent, baseHash ?? undefined, true) : await fsWrite(wsForFs!, targetPath, patched.modifiedContent, baseHash ?? undefined);
  ...
  const runRecipeChecks = useCallback(
    async (finalText: string): Promise<boolean> => {
      ...
      for (const check of checks) {
        ...
        let approval: { approved: boolean; token?: string } = { approved: false };
        try {
          approval = await autoApproveShell({ command, cwd: undefined });
        } catch {
          approval = { approved: false };
        }
        if (!approval.approved) {
  ...
  const streamingReasoning = isStreamingAssistant
    ? String((lastMsg as unknown as { reasoning?: string }).reasoning ?? '')
    : '';
  ...
  return {
    ...
    composerApiRef,
    takeBackQueued,
    workspaceReconnectRequired,
    dismissedReconnect,
    setDismissedReconnect,
    reconnectWorkspace,
    showNotice,
    plan,
    planHidden,
