...
describe('RED TEAM catalog — getToolEntry / resolveToolEntry trên mọi tên và alias', () => {
  it('getToolEntry trả đúng entry cho đủ 24 tên chính', () => {
...
describe('RED TEAM catalog — TOOL_CATEGORY_MAP phủ một-một mọi tên', () => {
  ...
  it('toolsByCategory phân hoạch đúng: đủ 24, mỗi entry đúng 1 nhóm', () => {
...
describe('RED TEAM — bản export suy sinh của agent-tools không drift khỏi catalog', () => {
  ...
  it('TOOL_SHORT_LABELS có đủ 24 key, nhãn khớp catalog từng entry', () => {
    expect(Object.keys(TOOL_SHORT_LABELS)).toHaveLength(24);
    for (const entry of TOOL_CATALOG) {
...
describe('RED TEAM — formatToolProtocolManual mô tả đủ 24 tool', () => {
