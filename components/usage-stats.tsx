  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
          ...
          {RANGES.map((r) => (
            <button
              ...
              className={`rounded-md px-2.5 py-1 text-[11px] font-medium transition ${
                days === r.days ? 'bg-white text-[#0A7E8C] shadow-sm' : 'text-zinc-500 hover:text-zinc-700'
              }`}
            >
              {r.label}
            </button>
        ...
        <span className="text-[11px] text-zinc-500">{formatTokens(summary.messages)} tin nhắn</span>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-xl border border-zinc-200 bg-white p-2.5">
              <div className="text-[10px] uppercase tracking-wide text-zinc-500">Token vào</div>
              <div className="text-sm font-semibold text-zinc-800">{formatTokens(summary.promptTokens)}</div>
            </div>
            <div className="rounded-xl border border-zinc-200 bg-white p-2.5">
              <div className="text-[10px] uppercase tracking-wide text-zinc-500">Token ra</div>
              <div className="text-sm font-semibold text-zinc-800">{formatTokens(summary.completionTokens)}</div>
            </div>
            <div className="rounded-xl border border-zinc-200 bg-white p-2.5">
              <div className="text-[10px] uppercase tracking-wide text-zinc-500">Tổng cộng</div>
              <div className="text-sm font-semibold text-[#0A7E8C]">{formatTokens(totalAll)}</div>
            </div>
          </div>
          ...
          {summary.byDay.length > 1 && (
            <div className="rounded-xl border border-zinc-200 bg-white p-3">
              <div className="mb-2 text-[11px] font-medium text-zinc-600">Theo ngày</div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex w-full flex-col overflow-hidden rounded-[3px]" style={{ height: `${h}%` }}>
                        <div className="w-full bg-[#4ECB71]/70" style={{ height: `${100 - inPct}%` }} />
                        <div className="w-full bg-[#0A7E8C]" style={{ height: `${inPct}%` }} />
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="space-y-1.5 rounded-xl border border-zinc-200 bg-white p-3">
            <div className="mb-1 text-[11px] font-medium text-zinc-600">Theo model</div>
            {summary.byModel.map((m) => {
              const t = m.promptTokens + m.completionTokens;
              return (
                <div key={m.model} className="space-y-0.5">
                  <div className="flex items-baseline justify-between gap-2 text-[11px]">
                    <span className="min-w-0 truncate font-mono text-zinc-700">{m.model}</span>
                  ...
                  <div className="h-1.5 overflow-hidden rounded-full bg-zinc-100">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-[#0A7E8C] to-[#4ECB71]"
